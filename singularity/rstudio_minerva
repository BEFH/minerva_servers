...
elif grep -q 'Host minerva[[:space:]]*$' $HOME/.ssh/config &> /dev/null; then
  S_HOSTNAME="minerva"
...
cm_enabled=$(! [ -f $HOME/.ssh/config ] ||
             ! grep -q "ControlPath" $HOME/.ssh/config &> /dev/null ||
             echo 1 || echo 0)
...
if [[ "$S_HOSTNAME" == "minervarun" ]]; then
  eval "$S_CONTENTS_MKDIRS"
else
  ssh $S_HOSTNAME "bash -c '$S_CONTENTS_MKDIRS'"
fi
...
create_rstudio_passwd_on_file () {
  ...
  # write to file
  if [[ "$S_HOSTNAME" == "minervarun" ]]; then
    mkdir -p $(dirname $S_FILE_PASSWORD)
    printf "# Password generated by rstudio_minerva\n" > $S_FILE_PASSWORD
    printf "export PASSWORD=$password\n" >> $S_FILE_PASSWORD
  else
...
check_set_runtime_password () {
  if [[ "$S_HOSTNAME" == "minervarun" ]]; then
    if ! test -e "$S_FILE_PASSWORD" ; then
      echoinfo "$S_FILE_PASSWORD is not found, we need to create it."
      create_rstudio_passwd_on_file
    fi
  elif ! ssh $S_HOSTNAME "test -e \"$S_FILE_PASSWORD\""; then
    echoinfo "$S_FILE_PASSWORD is not found, we need to create it."
    create_rstudio_passwd_on_file
  fi
}
...
S_CONTENTS_MKCONFS=$(cat <<EOF
if ! test -e "$RS_WORKDIR/database.conf"; then
  printf "provider=sqlite\n" > $RS_WORKDIR/database.conf
  printf "directory=/var/lib/rstudio-server\n" >> $RS_WORKDIR/database.conf
fi
if ! test -e "$RS_WORKDIR/rsession.conf"; then
  printf "# add your configuration here\n" > $RS_WORKDIR/rsession.conf
  printf "session-timeout-minutes=9000\n" >> $RS_WORKDIR/rsession.conf
  printf "copilot-enabled=1\n" >> $RS_WORKDIR/rsession.conf
  printf "copilot-proxy-url=http://172.28.7.1:3128\n" >> $RS_WORKDIR/rsession.conf
fi
if ! test -e "$RS_WORKDIR/rserver.conf"; then
  printf "# add your configuration here\n" > $RS_WORKDIR/rserver.conf
  printf "auth-timeout-minutes=0\n" >> $RS_WORKDIR/rserver.conf
  printf "auth-stay-signed-in-days=30\n" >> $RS_WORKDIR/rserver.conf
fi
EOF
)

if [[ "$S_HOSTNAME" == "minervarun" ]]; then
  eval "$S_CONTENTS_MKCONFS"
else
  ssh $S_HOSTNAME "bash -c '$S_CONTENTS_MKCONFS'"
fi

# check image

echoinfo "Checking for RStudio Server image"

RS_IMAGE_DLDEFAULT=0
if [[ $RS_IMAGE == "default" ]]; then
  RS_IMAGE="/sc/arion/work/${S_USERNAME}/rstudio_latest.sif"
  if [[ "$S_HOSTNAME" == "minervarun" ]]; then
    if ! test -e "$RS_IMAGE"; then
      RS_IMAGE_DLDEFAULT=1
    fi
  else
...
if [[ $RS_IMAGE_DLDEFAULT -eq 1 ]]; then
  image_url="library://befh/rstudio-server-conda/rstudio-server-conda:latest"
...
instructs_mrun() {
  ...
  echoinfo "http://localhost:LOCAL_PORT"
  echoinfo "Open the url in your browser and use the password to log in."
  echoinfo "The password is $S_PASSWORD"
}
...
if [ -f $S_RCI ]; then
  ...
  if [[ $RC_JOBSTATE == "running" ]]; then
    ...
    if [[ "$S_HOSTNAME" == "minervarun" ]]; then
      ...
      S_PASSWORD=$(sed -nE 's/^Password +: (.+)/\1/p' $S_RCI)
      instructs_mrun
      exit 0
    fi
...
if [[ "$S_HOSTNAME" == "minervarun" ]]; then
  eval "$S_CONTENTS_RMFILES"
else
  ssh $S_HOSTNAME "bash -c '$S_CONTENTS_RMFILES'"
fi
...
if [ "$S_NUM_CPU" -gt "64" ]; then
  echoerror "$S_NUM_CPU -> Larger than 64. No distributed memory supported, therefore the number of CPU cores needs to be smaller or equal to 128\n"
  display_help
fi
...
RS_CONDADIRS=$(echo "$S_CONDAINFO" | awk '
...
if [[ $RS_CONDAENV == "null" ]]; then
  ...
  if [[ $S_HOSTNAME == "minervarun" ]]; then
    RS_CONDAEP="$CONDA_DEFAULT_ENV $CONDA_PREFIX"
  else
...
R_LOG_LEVEL="info"
if [[ $S_DEBUG -eq 1 ]]; then
  echodebug "R Log level set to debug"
  R_LOG_LEVEL="debug"
fi

S_CONTENTS_LOGCONF=$(cat <<EOF
[*]
log-level=$R_LOG_LEVEL
logger-type=file
...
echodebug "Logging configuration contents:"
if [[ $S_DEBUG -eq 1 ]]; then
  echo "$S_CONTENTS_LOGCONF"
fi
...
S_CONTENTS_BASHPROF=$(cat <<EOF
PATH=\$(echo \$PATH |
  awk -v RS=':' '!/conda/ { printf "%s%s", sep, \$0; sep = ":" }')
...
alias minerva='conda deactivate; echo leaving sandbox; ssh -t li03c03 ijob'
EOF
)

echodebug "Bash profile contents:"
if [[ $S_DEBUG -eq 1 ]]; then
  echo "$S_CONTENTS_BASHPROF"
fi
...
#BSUB -oo $RS_WORKDIR/rstudio$S_SESSION_%J.out
#BSUB -eo $RS_WORKDIR/rstudio$S_SESSION_%J.err
#BSUB -L /bin/bash
EOF
)
...
singularity run \\
  ...
  --bind $RS_WORKDIR/rserver.conf:/etc/rstudio/rserver.conf \\
...
#  --bind /hpc/packages \\
#  --bind /etc/profile.d/00-modulepath.sh:/etc/profile.d/00-modulepath.sh \\
#  --bind /etc/profile.d/00-modulepath.csh:/etc/profile.d/00-modulepath.csh \\
#  --bind /etc/profile.d/lsf.sh:/etc/profile.d/lsf.sh \\
#  --bind /etc/profile.d/lsf.csh:/etc/profile.d/lsf.csh \\
#  --bind /etc/profile.d/mpi-selector.sh:/etc/profile.d/mpi-selector.sh \\
#  --bind /etc/profile.d/z00_lmod.sh:/etc/profile.d/z00_lmod.sh \\
#  --bind /usr/share/lmod/:/usr/share/lmod/:ro \\
#  --bind /hpc/lsf:/hpc/lsf:ro \\

echodebug "Adding conda and module binds to job script"

cmdstr=$(cat <<EOF
  echo
  for i in $(echo "$RS_CONDADIRS" | tr '\n' ' '); do
    if test -e "\$i"; then
      echo "  --bind \$i "
    fi
  done
...
if [[ $S_HOSTNAME == "minervarun" ]]; then
  S_CONTENTS_JOB+=$(bash -c "$cmdstr" | sed '/./ s|$|\\|')
else
  S_CONTENTS_JOB+=$(ssh $S_HOSTNAME "bash -c '$cmdstr'" | sed '/./ s|$|\\|')
fi
...
if [[ $S_HOSTNAME == "minervarun" ]]; then
  ...
  echodebug "Locally copying logconf, bashprof, and job files on minerva"
  echo "$S_CONTENTS_LOGCONF" > "$S_FILE_LOGCONF"
  echo "$S_CONTENTS_BASHPROF" > "$S_FILE_BASHPROF"
  echo "$S_CONTENTS_JOB" > "$S_FILE_JOB"
else
  # Copy the docstring to $S_FILE_LOGCONF remotely
  echodebug "Copying logconf, bashprof, and job files to $S_HOSTNAME"
  ssh -T $S_HOSTNAME "cat > '$S_FILE_LOGCONF'" <<< "$S_CONTENTS_LOGCONF"
  ssh -T $S_HOSTNAME "cat > '$S_FILE_BASHPROF'" <<< "$S_CONTENTS_BASHPROF"
  ssh -T $S_HOSTNAME "cat > '$S_FILE_JOB'" <<< "$S_CONTENTS_JOB"
fi
echodebug "Done copying logconf, bashprof, and job files"
...
if [[ $S_HOSTNAME == "minervarun" ]]; then
  ...
  S_LOCAL_PORT=$(find_port 8990)
else
  S_LOCAL_PORT=$(find_port 8999)
fi
